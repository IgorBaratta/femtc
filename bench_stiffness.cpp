...
void run(int degree, int Ndofs, std::string order_str, int Mb, int Nb)
{
  ...
  std::vector<T> G(num_cells * ndofs * geom_size, 0.0);
  std::generate(G.begin(), G.end(), []() { return std::rand() / (T)RAND_MAX; });
  ...
  {
    double t0 = MPI_Wtime();
    operators::stiffness_operator<T>(phi, U, W, G, num_cells, degree, order, Mb,
                                     Nb);
    double t1 = MPI_Wtime();
    MPI_Barrier(comm);
  ...
  double flops = 3 * 3 * 2 * (2.0 * m * n * k + n * k) * num_cells;

  // Compute memory access (U read, W write + read, G read)
  double mem_access = (U.size() + 2 * W.size() + G.size()) * sizeof(T);
