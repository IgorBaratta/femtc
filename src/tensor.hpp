/// @file
/// @brief This file contains classes for Tensor and Matrix, for
...
// template <TensorLayout in, TensorLayout out, std::size_t Na, std::size_t Nb,
// std::size_t Nc, typename T> void transpose(T *in_data, T *out_data)
// {
//     constexpr std::array<std::size_t, 3> offsets0 = get_offsets<in, Na, Nb,
//     Nc>(); constexpr std::array<std::size_t, 3> offsets1 = get_offsets<out,
//     Na, Nb, Nc>(); if constexpr (in == out)
...
//                     out_data[offsets1[0] * i + offsets1[1] * j + offsets1[2]
//                     * k] = in_data[offsets0[0] * i + offsets0[1] * j +
//                     offsets0[2] * k];
//     }
...
//             for (std::size_t j = 0; j < Nb; ++j)
//                 for (std::size_t k = 0; k < Nc; ++k)
//                     out_data[offsets1[0] * i + offsets1[1] * k + offsets1[2]
//                     * j] = in_data[offsets0[0] * i + offsets0[1] * j +
//                     offsets0[2] * k];
//     }
...
//             for (std::size_t j = 0; j < Nb; ++j)
//                 for (std::size_t k = 0; k < Nc; ++k)
//                     out_data[offsets1[0] * j + offsets1[1] * i + offsets1[2]
//                     * k] = in_data[offsets0[0] * i + offsets0[1] * j +
//                     offsets0[2] * k];
//     }
// }
...
// /// @tparam Nb The number of columns in the tensor.
// /// @tparam Nc The number of layers in the tensor.
// template <typename T, int Na, int Nb, int Nc, TensorLayout layout =
// default_layout> class Tensor
// {
// public:
...
//     constexpr static std::array<std::size_t, 3> offsets = get_offsets<layout,
//     Na, Nb, Nc>();
...
//     /// @brief Accesses elements in the tensor using row, column, and layer
//     indices.
...
//     inline constexpr T &operator()(std::size_t i, std::size_t j, std::size_t
//     k)
//     {
...
//     /// @brief Accesses elements in the tensor using row, column, and layer
//     indices (const version).
//     /// @param i The row index of the element.
//     /// @param j The column index of the element.
//     /// @param k The layer index of the element.
//     /// @return A const reference to the element at the specified indices.
//     inline constexpr const T &operator()(std::size_t i, std::size_t j,
//     std::size_t k) const
//     {
//         return data[i * offsets[0] + j * offsets[1] + k * offsets[2]];
//     }

//     /// @brief Accesses elements in the tensor using row, and linear column
//     and layer indices.
//     /// @param i The row index of the element.
...
//     /// @brief Accesses elements in the tensor such the first index
//     represents the row, and the
//     /// second index represents the linear of the two remaining dimensions.
//     E.g. for a tensor with
//     /// layout ijk, the first index represents the row, and the second index
//     represents the linear
//     /// column and layer indices. For a tensor with layout ikj, the first
//     index represents the row,
...
//     friend std::ostream &operator<<(std::ostream &os, const Matrix<T, N0, N1,
//     layout> &m)
//     {
//         for (int i = 0; i < N0; ++i)
//         {
