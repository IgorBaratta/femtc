...
void transpose(const T* __restrict__ A, T* __restrict__ B)
{
  for (int a = 0; a < Na; ++a)
    for (int b = 0; b < Nb; ++b)
...
/// @param[in] B tensor of shape (Nb, Nk) -> Shape (Nb, Nk) so that we
/// can transverse row-wise
/// @param[out] C tensor of shape (Na, Nb)
template <typename T, int Na, int Nb, int Nk>
void contract(const T* __restrict__ A, const T* __restrict__ B,
              T* __restrict__ C)
{
  for (int a = 0; a < Na; ++a)
    for (int b = 0; b < Nb; ++b)
...
void transpose(T* __restrict__ A, T* __restrict__ B)
{
  for (int a = 0; a < Na; a++)
    for (int b = 0; b < Nb; b++)
      for (int c = 0; c < Nc; c++)
...
/// Compute the tensor contraction C[a,b,c] <- A[a, k] B[k, b, c] as a
/// matrix- matrix multiplication C[a,{b, c}] = A[a, k] B[k, {b, c}] K
/// is the contraction index
...
void apply_contractions(const T* I0, const T* I1, const T* I2,
                        ...
                        Buffer<T, Na, Nb>& buffer)
{
  ...
  // [b1, a0, b2] <- [a0, b1, b2]
  transpose<T, Na, Nb, Nb, Nb, Nb * Na, 1>(buffer.T0.data(),
                                           buffer.T0_t.data());
  ...
  // [b2, a0, a1] <- [a1, a0, b2]
  transpose<T, Na, Na, Nb, 1, Na, Na * Na>(buffer.T1.data(),
                                           buffer.T1_t.data());
