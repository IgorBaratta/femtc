...
namespace linalg
{
    ...
    inline constexpr int get_offset0()
    {
    ...
    template <int N0, int N1, int N2, TensorLayout layout>
    inline constexpr int get_offset1()
    {
        if constexpr (layout == TensorLayout::ijk)
            return N2;
        else if constexpr (layout == TensorLayout::ikj)
    ...
    /// @param[in] layout tensor layout
    template <int N0, int N1, int N2, TensorLayout layout>
    inline constexpr int get_offset2()
    {
        if constexpr (layout == TensorLayout::ijk)
            return 1;
        else if constexpr (layout == TensorLayout::ikj)
    ...
    /// @param[out] B tensor of shape (N0, N1, N2) stored in P(I) order, lout layout
    /// Example: permute<double, 2, 3, 4, TensorLayout::ijk, TensorLayout::kij >(A, B);
    template <typename T, int N0, int N1, int N2, TensorLayout lin, TensorLayout lout>
    void permute(const T *in, T *out)
    {
        ...
        else if constexpr (lin == TensorLayout::ijk)
        {
            for (int i = 0; i < N0; i++)
                for (int j = 0; j < N1; j++)
                    for (int k = 0; k < N2; k++)
                        out[k * offout2 + j * offout1 + i * offout0] = in[i * offin0 + j * offin1 + k * offin2];
        }
